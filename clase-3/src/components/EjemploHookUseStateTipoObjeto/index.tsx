...
const EjemploHookUseStateTipoObjeto: FC = () => {
  const [objeto, setObjeto] = useState({
    nombre: "Yudith",
    contrasenia: "1234223",
  });

  const guardarNuevoNombre = (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const guardarNuevaContrasenia = (event: ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div>
      ...
      <input
        type="password"
        name="contrasenia"
        value={objeto.contrasenia}
        onChange={guardarNuevaContrasenia}
      />
    </div>
  );
};
